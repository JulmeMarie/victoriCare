...
public class UserServiceImpl implements IUserService {
	...
	public UserModel uploadImage(User connection, MultipartFile mFile) {
		if (!mFile.isEmpty()) {
			this.fileService.setS3BucketName(this.jwtUtils.getS3BucketName());
			final String fileToDelete = connection.getPhotoUser();
			String filename = fileService.save(EFolder.USER, mFile);
			connection.setPhotoUser(filename);
			connection.setUpdateAtUser(Date.from(Instant.now()));
			this.userRepository.save(connection);
			fileService.delete(fileToDelete);
		}
		fileService.setS3BucketUrl(this.jwtUtils.getS3BucketUrl());
		return UserModel.newInstance().init(fileService, connection);
	}
	...
	public UserModel validate(ValidateDTO validateRequest) {
		...
		Date now = Date.from(Instant.now());
		User author = this.userRepository.findById(validateRequest.getId())
