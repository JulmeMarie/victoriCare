...
public enum ERight {
	...
	ADMINISTRATOR, MODERATOR, USER, ANONYMOUS;
	...
	public static List<ERight> compute(ERight right) {
		...
		return Stream.of(ERight.values()).filter(r->{
			String rName = r.name();
			if(rName.equals(rightName) || rightName.equals(ERight.ADMINISTRATOR.name()) || (!rName.equals(ERight.ADMINISTRATOR.name()) && rightName.equals(ERight.MODERATOR.name())))
				return true;
