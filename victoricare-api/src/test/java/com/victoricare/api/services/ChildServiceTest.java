...
public class ChildServiceTest {
    ...
    public void doDeleteChildCareTaskMustBeOKTest() {
        ...
        willDoNothing().given(deletionHistoryService).doCreate(child, onlineUser.getId());
        willDoNothing().given(repository).delete(child);
        service.doDelete(deletionHistoryService, onlineUser, child.getId());
    }

    @Test
    @Order(4)
    public void doDeleteChildCareTaskNonExistingTest() {
        // precondition
        given(repository.findById(child.getId())).willReturn(Optional.empty());
