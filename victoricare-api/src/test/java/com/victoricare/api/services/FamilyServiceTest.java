...
public class FamilyServiceTest {
    ...
    public void doDeleteFamilyCareTaskMustBeOKTest() {
        ...
        willDoNothing().given(deletionHistoryService).doCreate(family, onlineUser.getId());
        willDoNothing().given(repository).delete(family);
        service.doDelete(deletionHistoryService, onlineUser, family.getId());
    }

    @Test
    @Order(4)
    public void doDeleteFamilyCareTaskNonExistingTest() {
        // precondition
        given(repository.findById(family.getId())).willReturn(Optional.empty());
