...
public class AddressServiceTest {
    ...
    private static final String ADDITIONAL = "88 Rue Vivaldi";
    ...
    public void setup() {
        address = Address.builder()
                ...
                .zipCode(ZIP_CODE)
                .country(COUNTRY)
                .addtionnal(ADDITIONAL)
                .creationAuthorId(USER_ID)
                .build();

        addressInputDTO = new AddressInputDTO();
        addressInputDTO.setStreet(STREET);
        addressInputDTO.setTown(TOWN);
        addressInputDTO.setZipCode(ZIP_CODE);
        addressInputDTO.setCountry(COUNTRY);
        addressInputDTO.setAddtionnal(ADDITIONAL);
    }
    ...
    public void deleteAddressTest() {
        ...
        given(repository.findById(address.getId())).willReturn(Optional.of(address));
        willDoNothing().given(deletionHistoryService).doCreate(address, user.getId());
        // precondition
        willDoNothing().given(repository).delete(address);

        // action
        service.doDelete(deletionHistoryService, USER_ID, address.getId());
    ...
    @Test
    @Order(10)
    public void deleteAddressThrowException() {
