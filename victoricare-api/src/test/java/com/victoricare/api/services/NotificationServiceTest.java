...
public class NotificationServiceTest {
    ...
    public void doDeleteNotificationCareTaskMustBeOKTest() {
        ...
        willDoNothing().given(deletionHistoryService).doCreate(notification, onlineUser.getId());
        willDoNothing().given(repository).delete(notification);
        service.doDelete(deletionHistoryService, onlineUser, notification.getId());
    }

    @Test
    @Order(4)
    public void doDeleteNotificationCareTaskNonExistingTest() {
        // precondition
        given(repository.findById(notification.getId())).willReturn(Optional.empty());
