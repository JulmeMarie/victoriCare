...
public class ChildCareTaskServiceTest {
    ...
    public void doDeleteChildCareTaskMustBeOKTest() {
        // precondition
        given(repository.findById(childCareTask.getId())).willReturn(Optional.of(childCareTask));
        willDoNothing().given(deletionHistoryService).doCreate(childCareTask, onlineUser.getId());
        willDoNothing().given(repository).delete(childCareTask);
        service.doDelete(deletionHistoryService, onlineUser, childCareTask.getId());
    }
    ...
    public void doDeleteChildCareTaskNonExistingTest() {
        // precondition
        given(repository.findById(childCareTask.getId())).willReturn(Optional.empty());
