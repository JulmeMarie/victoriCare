...
public class ChildCareServiceTest {
    ...
    public void doDeleteChildCareMustBeOKTest() {
        // precondition
        given(repository.findById(childCare.getId())).willReturn(Optional.of(childCare));
        willDoNothing().given(deletionHistoryService).doCreate(childCare, onlineUser.getId());
        willDoNothing().given(repository).delete(childCare);
        service.doDelete(deletionHistoryService, onlineUser, childCare.getId());
    }

    @Test
    @Order(4)
    public void doDeleteChildCareNonExistingTest() {
        // precondition
        given(repository.findById(childCare.getId())).willReturn(Optional.empty());
