...
public class SignUpServiceTest {
    ...
    @Test
    @Order(2)
    public void doDeleteUnexistingSignUpMustThrowExceptionTest() {
        // precondition
        given(repository.findById(signUp.getId())).willReturn(Optional.empty());
        assertThrows(ResourceNotFoundException.class, () -> service.doDelete(deletionHistoryService, user,
                signUp.getId()));
    }
    ...
    public void doDeleteSignUpMustBeOKTest() {
        // precondition
        given(repository.findById(signUp.getId())).willReturn(Optional.of(signUp));
        willDoNothing().given(deletionHistoryService).doCreate(signUp, user.getId());
        willDoNothing().given(repository).delete(signUp);
        service.doDelete(deletionHistoryService, user, signUp.getId());
    }
